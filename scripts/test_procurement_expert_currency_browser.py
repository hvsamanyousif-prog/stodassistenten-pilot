"""Focused browser regression for currency-version source-risk retention.

Offline Chromium/WebKit evidence only. Network is disabled. Manual evidence marking is
...
OUT = Path(sys.argv[2]) if len(sys.argv) > 2 else Path('procurement-currency-browser.json')
...
CASES = [
    {
        'id': 'changed-eur-version-risk-survives-manual-evidence',
        'text': '\n'.join([
    ...
    {
        'id': 'unchanged-eur-does-not-fabricate-risk',
        'text': '\n'.join([
            'Version 1: Fast pris 100 000 EUR ska gälla.',
...
def install_offline(page, context):
    ...
    page.evaluate("() => { window.fetch = () => Promise.reject(new Error('Network disabled in currency regression')); }")
...
def run_case(page, case):
    ...
    check(categories == ['commercial', 'commercial'], f"{case['id']}: unexpected categories {categories}")
    ...
    if case['expect_conflict']:
        check('motstridiga prisversioner' in overview, f"{case['id']}: EUR version risk disappeared after evidence=yes")
        check('källa rad 1' in overview and 'källa rad 2' in overview, f"{case['id']}: source trace missing from priority overview")
        check('alla kravrader är genomgångna av dig' not in overview, f"{case['id']}: false calm completion shown")
        check('börja med de markerade riskerna' in overview, f"{case['id']}: unresolved source risk not prioritized")
        check('2 osäkra/ej bedömda' in summary, f"{case['id']}: summary no longer reflects unresolved flagged rows: {summary}")
    else:
        check('motstridiga prisversioner' not in overview, f"{case['id']}: unchanged EUR fabricated version conflict")
        check('alla kravrader är genomgångna av dig' in overview, f"{case['id']}: unchanged EUR control did not reach risk-free reviewed state")
...
report = {
    'scope': 'Focused offline explicit-currency commercial version and calm-overview source-risk regression. Network disabled; manual evidence marking is not source verification. No live Pages, storage, legal, physical Safari/iPad/iPhone or assistive-technology claim.',
    'engine': ENGINE,
