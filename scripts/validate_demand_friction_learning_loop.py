...
Joins all versioned qualitative demand/friction signal packs and all versioned
signal->regression mapping packs to the single scenario laboratory. Signals are
discovery/prioritization evidence only; they never become an eligibility or
VERIFIED truth source.
"""
import json
...
MAP_PATHS = sorted(EVAL.glob("demand_friction_regression_map_v*.json"))

assert len(SIGNAL_PATHS) >= 3, "expected versioned demand/friction signal packs"
assert MAP_PATHS, "demand/friction regression maps missing"
...
assert len(signals) >= 8, f"demand/friction system requires at least 8 prioritized needs, got {len(signals)}"
assert sum(s["priority_band"] == "HIGH" for s in signals.values()) >= 5, "expected multiple high-priority weak-link signals"
...
for path in MAP_PATHS:
    mapping_pack = json.loads(path.read_text(encoding="utf-8"))
    for mapping in mapping_pack.get("mappings", []):
        ...
        assert sid not in mappings, f"duplicate mapping across mapping packs: {sid}"
...
required_links = {
    ...
    "df-partialsick-vab-hours-overlap-v01": "lab-employee-partialsick-vab-v16-01",
    "df-property-housing-adaptation-owner-role-v01": "lab-property-housing-adaptation-takeover-v17-01",
    "df-dental-67-strengthened-protection-v01": "lab-older-dental-67-v19-01",
}
...
print(
    ...
    f"{len(scenario_ids)} canonical scenarios available; {len(MAP_PATHS)} mapping packs)"
)
