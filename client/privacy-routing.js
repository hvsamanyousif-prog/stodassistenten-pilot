(() => {
  ...
  function boundedHousingNeed(text){
    const x=lower(text);
    return /(?:\bhyran\b|\b(?:hög|dyr)\s+hyra\b|\bhyra\b(?=\s*(?:och|,|\.|$))|\b(?:boende|bostads)kostnad(?:en|er|erna)?\b|\bbostad(?:en)?\b|\brent\b|(?:ال)?إيجار\s+(?:مرتفع|عال(?:ي|ية)?|غالي|غالية)|إيجار(?:ه|ها)\s+مرتفع|بعد\s+الإيجار|(?:تكلفة|تكاليف)\s+السكن|السكن|اجاره\s+(?:بالا(?:یی)?|زیاد|سنگین|گران)|اجاره(?:‌اش|\s+اش)\s+بالاست|(?:بعد|پس)\s+از\s+اجاره|مسکن)/.test(x);
  }
  ...
  function explicitlyCorrectsActor(text,actor){
    ...
    const patterns={
      ...
      fa:{
        ...
        company:/دیگر صاحب شرکت نیستم|صاحب شرکت نیستم|نه برای شرکت (?:من|ما)|برای شرکت (?:من|ما) نیست/,
