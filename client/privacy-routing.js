(() => {
  ...
  const FUNDING_INTENT_PATTERNS=[
    ...
    ['funding',/pengar\s+att\s+sök|sök(?:a|er)?\s+pengar|(?:\bfond(?:er)?\b\s+att\s+sök|\bfonder\b\s+(?:kan\s+jag|jag\s+kan)\s+sök(?:a|er)?|sök(?:a|er)?\s+(?:en\s+)?fond(?:er)?\b)|bidrag\s+att\s+sök|sök(?:a|er)?\s+bidrag|finansiering\s+att\s+sök|دعم(?:اً|ًا|ا)?\s+مالي(?:اً|ًا|ا)?|(?:ال)?مساعد(?:ة|ات)\s+(?:ال)?مالية|تمويل|کمک مالی|حمایت مالی|بودجه/u]
  ];
