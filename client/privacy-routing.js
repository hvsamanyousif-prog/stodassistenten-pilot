(() => {
  ...
  function actorCandidatesFromText(text){
    ...
    add('company',/\b(?:jag\s+är\s+företagare|jag\s+driver\s+(?:ett\s+|en\s+)?företag|mitt\s+företag|vårt\s+företag)\b|(?:^|[.!?]\s*)driver\s+(?:ett\s+|en\s+)?företag\b|(?:^|[^\p{L}\p{N}])(?:لدي\s+شركة|لدينا\s+شركة|شركتي|شركتنا|نحن\s+شركة|أنا\s+صاحب(?:ة)?\s+شركة)(?=$|[^\p{L}\p{N}])|(?:^|[^\p{L}\p{N}])(?:شرکت\s+من|کسب.?وکار\s+من|من\s+(?:یک\s+)?کسب.?وکار\s+دارم)(?=$|[^\p{L}\p{N}])/u);
