(function(root){
'use strict';
const APP_VERSION='procurement-expert-0.2.50';
...
function materialEvidenceObjectCount(line){
 ...
 const requestedStaffReferencePair=/\b(?:ska|skall|måste)\b[^.;]{0,100}\b(?:innehålla|omfatta|inkludera|bifoga|lämna|redovisa|skicka in|ge in)\b[^.;]{0,100}\bcv\b\s+(?:för|avseende)\s+\b(?:arbetsledar(?:e|en)|projektledar(?:e|en)|uppdragsledar(?:e|en)|nyckelperson(?:en|er|erna)?|specialist(?:en|er|erna)?)\b[^.;]{0,80}\b(?:och|samt)\b\s+(?:en|ett)?\s*\b(?:lista|förteckning)\b[^.;]{0,50}\b(?:med\s+)?(?:minst\s+)?(?:\d+|ett|en|två|tre|fyra)?\s*referens(?:uppdrag|er)?\b(?=\s*[.;]|$)/.test(t);
 const families=[
 ...
 return families.filter(pattern=>pattern.test(t)).length+(distinctCertificatePair?1:0)+(distinctReferencePair?1:0)+(distinctNamedRoleCompetencePair?1:0)+(distinctRequestedDeliverablePair?2:0)+(requestedStaffReferencePair?2:0);
}
function materialClauseCount(line){
 ...
 if(count<2){
   ...
   const clarificationEvent=/\b(?:frågor?|förtydliganden?)\b[^.;]{0,100}\b(?:ska\s+)?(?:lämnas|ställas|inkomma|begäras)\b[^.;]{0,80}\b(?:senast|sista dag)\b/.test(t);
   const participationEvent=participationApplicationTiming(t);
...
function materialBoundCrossLineCondition(part){
 const t=normalized(part);
 return formalCrossLineCondition(t)||/^vid användning av\b/.test(t)||/^undantag från kravet på(?:\s|$)/.test(t)||/^med undantag för(?:\s|$)/.test(t);
}
function sharesCrossLineMaterialFamily(leftText,rightText){
 const left=normalized(leftText),right=normalized(rightText);
 const families=[
   /\b(?:ansvarsförsäkring|försäkring)\b/,
   /\b(?:certifikat|certifiering|certifierad|iso\s*\d{3,5})\b/,
   /\breferens(?:uppdrag|er)?\b/,
   /\b(?:omsättning|ekonomisk|finansiell)\b/,
   /\b(?:kvalitetsledningssystem|miljöledningssystem|ledningssystem)\b/,
   /\b(?:behörighet|behörig|bas-p|bas-u)\b/,
   /\b(?:verktyg|maskiner?|utrustning)\b/,
   /\b(?:arbetsmiljöplan|arbetsmiljökrav|säkerhetsföreskrifter)\b/,
   /\b(?:fast pris|timpris|anbudspris|prisbilaga)\b/
 ];
